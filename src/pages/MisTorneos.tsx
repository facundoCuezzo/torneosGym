...
function MisTorneos() {
  ...
  const { handleGetScoresByGym, scores, loading: loadingScores, scoresPagination } = useScores();
  ...
  const handleSubmit = (values: FilterScores) => {
    ...
    handleGetScoresByGym({ ...values, id_tournament: selectedTournament }, actualPage);
  };

  const handlePageChange = async (page: number) => {
    if (filters) {
      await handleGetScoresByGym(
